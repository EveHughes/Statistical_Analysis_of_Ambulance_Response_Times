...
Since the data set is rather large the main goal when cleaning the data was to reduce the size. During the creation of the raw data set the Field Names, Desctiption/Definition, and Comments/Examples columns were added as part of the process combining the data for all 5 years. However, we don't use these columns within our numerical analysis so they were removed within the data cleaning to reduce the data size. Also, the ID and forward_sortation-area were removed as they didn't provide us with any useful information. The cleaned data is shown in @tbl-cleaned-data-1.
...
As we are interested in understanding the reason behind the reduced ambulance availability the only data types for which we can compute useful statistics are: number of dispatches per year, incident_type, priority_number, and units_arrived_at_scene. @tbl-cleaned-data-summary shows a brief summary of the data columns we are interested in, the summary of these calculated statistics with mean for continuous data and mode for categorical data. The total number of dispatches per year is visualized in @fig-year-vs-priority. on average the Toronto Paramedic Service's received 285 000 calls or 780 calls per day, with an average of 1.1 units sent per call that is approximately 860 units dispatched per day.
...
@fig-year-vs-1-unit-arrived and @fig-year-vs-2-units-arrived show how the number of units dispatched per year changed, with an overall positive trend until 2020 when the number dropped drastically inline with COVID-19 as mentioned previously. After which the initial drop numbers jump back up and resume the positive trend. It is worth noting we only look at 1 and 2 units dispatched as those are by far the most common dispatch numbers and we can see they both follow a similar trend.
...
We have seen from the results section that there was a significant drop in the number of calls Toronto Paramedic Services received during the pandemic and a steady increase afterwards. However, this doesn't explain the increased wait time as the number of calls is still less than pre-COVID, but if we take into account the full context with many health care workers feelings burned and considering changing career paths @citeStatsCan. This coupled with the increase in calls and units dispatched after the COVID-19 pandemic ended resulted in the Toronto Paramedic Services being under equipped to deal with the increase in call volume. Furthermore, the increase in reports of priority 1 and 3, which are life-threatening or possibly life-threatening situations means that there was a need for experienced medical personnel at a time when there was a shortage. All these factors combined most likely resulted in longer wait times for Torontonians in need of medical assistance.

# Weaknesses and next steps {#sec-nextsteps}

A possible next step could be including the dispatch arrival time and seeing if there is any change within the time paramedic units take to respond to a request. As the data set I analyzed only contained the dispatch time I wasn't able to perform this calculation, however if completed we could get information on if the increased call wait times resulted in increased response times aswell. 
\newpage
